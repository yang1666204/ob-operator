...
tenantTopoData: API.ReplicaDetailType[])
{
  ...
  if (targetZone) {
    return {
      ...
      minIops: targetZone.minIops >= MAX_IOPS ? intl.formatMessage({ id: "src.components.TopoComponent.69696489", defaultMessage: "无限制" }) : targetZone.minIops,
      maxIops: targetZone.maxIops >= MAX_IOPS ? intl.formatMessage({ id: "src.components.TopoComponent.C6ED25B9", defaultMessage: "无限制" }) : targetZone.maxIops
    };
  }
  return;
}
